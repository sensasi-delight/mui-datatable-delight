...
class TableBody extends React.Component {
    ...
    handleRowSelect = (data, event) => {
        ...
        if (
            shiftKey &&
            previousSelectedRow &&
            previousSelectedRow.index < this.props.data.length
        ) {
            ...
            let selectedRows = structuredClone(this.props.selectedRows)
    ...
    handleRowClick = (row, data, event) => {
        ...
        if (
            ...
            (event.target.nodeName === 'path' &&
                event.target.parentNode.id === 'expandable-button')
        ) {
            return
        }
        ...
        if (
            event.target.id &&
            event.target.id.startsWith('MUIDataTableSelectCell')
        )
            return
                ...
                {tableRows && tableRows.length > 0 ? (
                    tableRows.map((data, rowIndex) => {
                        ...
                        if (options.customRowRender) {
                            return options.customRowRender(
                                row,
                                dataIndex,
                                rowIndex
                            )
                        ...
                        let isRowSelected =
                            options.selectableRows !== 'none'
                                ? this.isRowSelected(dataIndex)
                                : false
                        let isRowSelectable = this.isRowSelectable(dataIndex)
                        let bodyClasses = options.setRowProps
                        ...
                        return (
                            <React.Fragment key={rowIndex}>
                                <TableBodyRow
                                    ...
                                    data-testid={
                                        'MUIDataTableBodyRow-' + dataIndex
                                    }
                                    id={`MUIDataTableBodyRow-${tableId}-${dataIndex}`}
                                >
                                    <TableSelectCell
                                        onChange={this.handleRowSelect.bind(
                                            null,
                                            {
                                        ...
                                        hideExpandButton={
                                            !this.isRowExpandable(dataIndex) &&
...
export default withStyles(TableBody, defaultBodyStyles, {
