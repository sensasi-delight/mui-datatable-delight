...
class MUIDataTableClass extends React.Component {
  ...
  handleOptionDeprecation = props => {
    ...
    if (Object.values) {
      if (
        Object.values(STP).indexOf(this.options.selectToolbarPlacement) === -1
      ) {
        this.warnDep(
  ...
  toggleViewColumn = index => {
    this.setState(
      ...
      () => {
        ...
        var cb =
          this.options.onViewColumnsChange || this.options.onColumnViewChange
    ...
    if (expandedRowsData.length > 0) {
      ...
      for (let ii = expandedRowsData.length - 1; ii >= 0; ii--) {
        ...
        if (
          !isRowExpandable ||
          (isRowExpandable &&
            isRowExpandable(item.dataIndex, this.state.expandedRows))
        ) {
    ...
    this.setState(
      ...
      () => {
        ...
        if (this.options.onRowExpansionChange || this.options.onRowsExpand) {
          let expandCallback =
            this.options.onRowExpansionChange || this.options.onRowsExpand
          expandCallback(
  ...
  render() {
    ...
    const TableToolbarSelectComponent =
      TableToolbarSelect || DefaultTableToolbarSelect
    ...
    return (
      ...
      >
        ...
        {(selectedRows.data.length === 0 ||
          [STP.ABOVE, STP.NONE].indexOf(this.options.selectToolbarPlacement) !==
            -1) &&
          showToolbar && (
