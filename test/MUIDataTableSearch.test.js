...
describe('<TableSearch />', function () {
    ...
    it('should change search bar text when searchText changes', () => {
        ...
        const actualResult = mountWrapper
            .setProps({ searchText: 'nextText' })
            .update()
        assert.strictEqual(actualResult.length, 1)
        assert.strictEqual(
            actualResult.find(TextField).props().value,
            'nextText'
        )
    })
    ...
    it('should render a search bar with placeholder when searchPlaceholder is set', () => {
        const options = {
            ...
            searchPlaceholder: 'TestingPlaceholder'
        }
        const onSearch = () => {}
        const onHide = () => {}
        ...
        assert.strictEqual(actualResult.length, 1)
        assert.strictEqual(
            actualResult.props().placeholder,
            'TestingPlaceholder'
        )
    })
    ...
    it('should hide the search bar when hitting the ESCAPE key', () => {
        ...
        const mountWrapper = mount(
            <TableSearch onHide={onHide} options={options} />,
            { attachTo: document.body }
        )

        simulant.fire(document.body.querySelector('input'), 'keydown', {
            keyCode: 27
        })
