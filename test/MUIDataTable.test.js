...
describe('<MUIDataTable />', function () {
  ...
  before(() => {
    columns = [
      ...
      'Company',
      {
        name: 'City',
        label: 'City Label',
        options: { customBodyRender: renderCities, filterType: 'textField' }
      },
      {
        name: 'State',
        options: {
  ...
  it('should correctly build internal table data and displayData structure with enabled nested data custom marker (option enableNestedDataAccess : "/OK/" )', () => {
    ...
    const displayData = JSON.stringify([
      ...
      {
        data: ['Houston, James', 'Test Corp', 'Dallas', 'TX', undefined],
        dataIndex: 3
      }
    ])
    const shallowWrapper = shallow(
...
*/
  ...
  it('should add custom props to table if setTableProps provided', () => {
    const options = {
      setTableProps: stub().returns({ myProp: 'test', className: 'testClass' })
    }
    const fullWrapper = mount(
      <MUIDataTable columns={columns} data={[]} options={options} />
    )
  ...
  it('should correctly call consoleWarnings', () => {
    ...
    oldResponsiveOptions.forEach(responsive => {
      ...
      const shallowWrapper = shallow(
        <MUIDataTable columns={columns} data={data} options={options2} />
      ).dive()
      const instance = shallowWrapper.instance()
    })
  ...
  it('should call onRowExpansionChange when row is expanded or collapsed', () => {
    ...
    assert(
      options.onRowExpansionChange.calledWith([{ index: 2, dataIndex: 2 }], [])
    )
  })
  ...
  it('should call onRowSelectionChange when row is selected or unselected', () => {
    ...
    assert(
      options.onRowSelectionChange.calledWith([{ index: 2, dataIndex: 2 }], [])
    )
  })
