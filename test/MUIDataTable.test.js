...
describe('<MUIDataTable />', function () {
    ...
    let renderCities = (value, tableMeta, updateValueFn) => (
        <Cities
            ...
            change={event => updateValueFn(event)}
        />
    )
        ...
        displayData = JSON.stringify([
            {
                data: [
                    'James, Joe',
                    'Test Corp',
                    renderCities('Yonkers', { rowIndex: 0 }),
                    'NY',
                    undefined
                ],
                dataIndex: 0
            },
            {
                data: [
                    'Walsh, John',
                    'Test Corp',
                    renderCities('Hartford', { rowIndex: 1 }),
                    null,
                    undefined
                ],
                dataIndex: 1
            ...
            },
            {
                data: [
                    'Houston, James',
                    'Test Corp',
                    renderCities('Dallas', { rowIndex: 3 }),
                    'TX',
                    undefined
                ],
                dataIndex: 3
    ...
    it('should correctly rebuild internal columns data structure', () => {
        ...
        shallowWrapper.setProps({
            columns: [
                ...
                {
                    name: 'Empty',
                    options: { empty: true, filterType: 'checkbox' }
                }
            ]
        })
    ...
    it('should correctly build internal table data and displayData structure when using nested data (option enableNestedDataAccess omitted )', () => {
        ...
        const data = [
            {
                Name: 'Joe James',
                Company: 'Test Corp',
                Location: { City: 'Yonkers', State: 'NY' }
            },
            {
                Name: 'John Walsh',
                Company: 'Test Corp',
                Location: { City: 'Hartford', State: null }
            },
            {
                Name: 'Bob Herm',
                Company: 'Test Corp',
                Location: { Town: 'Tampa', State: 'FL' }
            },
            {
                Name: 'James Houston',
                Company: 'Test Corp',
        ...
        const displayData = JSON.stringify([
            {
                data: ['James, Joe', 'Test Corp', null, null, null],
                dataIndex: 0
            },
            {
                data: ['Walsh, John', 'Test Corp', null, null, null],
                dataIndex: 1
            },
            {
                data: ['Herm, Bob', 'Test Corp', null, null, null],
                dataIndex: 2
            },
            {
                data: ['Houston, James', 'Test Corp', null, null, null],
                dataIndex: 3
            }
        ])
        const shallowWrapper = shallow(
    ...
    it('should correctly build internal table data and displayData structure with enabled nested data custom marker (option enableNestedDataAccess : "/OK/" )', () => {
        ...
        const displayData = JSON.stringify([
            {
                data: ['James, Joe', 'Test Corp', 'Yonkers', 'NY', undefined],
                dataIndex: 0
            },
            {
                data: ['Walsh, John', 'Test Corp', 'Hartford', null, undefined],
                dataIndex: 1
            },
            {
                data: ['Herm, Bob', 'Test Corp', undefined, 'FL', undefined],
                dataIndex: 2
            },
            {
                data: [
    ...
    it('should correctly build internal table data and displayData structure with disabled nested data (option enableNestedDataAccess : "" )', () => {
        ...
        const displayData = JSON.stringify([
            {
                data: ['James, Joe', 'Test Corp', null, null, undefined],
                dataIndex: 0
            },
            {
                data: ['Walsh, John', 'Test Corp', null, null, undefined],
                dataIndex: 1
            },
            {
                data: ['Herm, Bob', 'Test Corp', null, null, undefined],
                dataIndex: 2
            },
            {
                data: ['Houston, James', 'Test Corp', null, null, undefined],
                dataIndex: 3
            }
        ])
        const shallowWrapper = shallow(
    ...
    it('should correctly set tableId', () => {
        const fullWrapper = mount(
            <MUIDataTable
                columns={columns}
                data={[]}
                options={{ tableId: 'myTable123' }}
            />
        )
        assert.strictEqual(
            fullWrapper.find('[data-tableid="myTable123"]').length,
            12
        )
    })
    ...
    it('should correctly re-build internal table data and displayData structure with prop change', () => {
        ...
        const expectedResult = [
            {
                index: 0,
                data: ['testing', 'Test Corp', 'Yonkers', 'NY', undefined]
            },
            {
                index: 1,
                data: ['John Walsh', 'Test Corp', 'Hartford', null, undefined]
            },
            {
                index: 2,
                data: ['Bob Herm', 'Test Corp', 'Tampa', 'FL', undefined]
            },
            {
                index: 3,
                data: ['James Houston', 'Test Corp', 'Dallas', 'TX', undefined]
            }
        ]
    ...
    it('should correctly sort', () => {
        ...
        const fetchedDD1 = JSON.stringify(
            wrapper.childAt(0).state('displayData')
        )

        wrapper.find('[data-testid="headcol-0"]').at(0).simulate('click')
        const fetchedDD2 = JSON.stringify(
            wrapper.childAt(0).state('displayData')
        )

        wrapper.find('[data-testid="headcol-0"]').at(0).simulate('click')
        const fetchedDD3 = JSON.stringify(
            wrapper.childAt(0).state('displayData')
        )

        wrapper.find('[data-testid="headcol-0"]').at(0).simulate('click')
        const fetchedDD4 = JSON.stringify(
            wrapper.childAt(0).state('displayData')
        )
...
*/
    ...
    it('should not re-build internal table data and displayData structure with no prop change to data or columns', () => {
        const setTableDataSpy = spy(
            MUIDataTable.Naked.prototype,
            'setTableData'
        )
        const mountWrapper = mount(
            shallow(<MUIDataTable columns={columns} data={data} />).get(0)
        )
    ...
    it('should apply columns prop change for filterList', () => {
        const mountShallowWrapper = mount(
            shallow(<MUIDataTable columns={columns} data={data} />).get(0)
        )
        ...
        instance.setTableData(
            mountShallowWrapper.props(),
            1 /* instance.TABLE_LOAD.INITIAL */
        )
    ...
    it('should create Chip with custom label when filterList and customFilterListOptions are populated', () => {
        ...
        const filterListRenderers = columns.map(c => {
            return c.options &&
                c.options.customFilterListOptions &&
    ...
    it('should have the proper type in onFilterChange when calling resetFilters method', () => {
        let type
        const options = {
            onFilterChange: (changedColumn, filterList, changeType) =>
                (type = changeType)
        }
    ...
    it('should properly set searchText when hiding the search bar', () => {
        ...
        const instanceTableToolbar = shallowWrapperTableToolbar
            .dive()
            .instance()
        ...
        const expectedResult = JSON.stringify([
            ...
            },
            {
                data: [
                    'Houston, James',
                    'Test Corp',
                    renderCities('Dallas', { rowIndex: 1 }),
                    'TX',
                    undefined
                ],
                dataIndex: 3
            },
            {
                data: [
                    'James, Joe',
                    'Test Corp',
                    renderCities('Yonkers', { rowIndex: 2 }),
                    'NY',
                    undefined
                ],
                dataIndex: 0
            },
            {
                data: [
                    'Walsh, John',
                    'Test Corp',
                    renderCities('Hartford', { rowIndex: 3 }),
                    null,
                    undefined
                ],
                dataIndex: 1
        ...
        const expectedResult = JSON.stringify([
            {
                data: [
                    'Walsh, John',
                    'Test Corp',
                    renderCities('Hartford', { rowIndex: 0 }),
                    null,
                    undefined
                ],
                dataIndex: 1
            },
            {
                data: [
                    'James, Joe',
                    'Test Corp',
                    renderCities('Yonkers', { rowIndex: 1 }),
                    'NY',
                    undefined
                ],
                dataIndex: 0
            },
            {
                data: [
                    'Houston, James',
                    'Test Corp',
                    renderCities('Dallas', { rowIndex: 2 }),
                    'TX',
                    undefined
                ],
                dataIndex: 3
        ...
        const expectedResult = JSON.stringify([
            {
                data: [
                    'Houston, James',
                    'Test Corp',
                    renderCities('Dallas', { rowIndex: 0 }),
                    'TX',
                    undefined
                ],
                dataIndex: 3
    ...
    it('should get displayable data when calling getDisplayData method', () => {
        ...
        const actualResult = instance.getDisplayData(
            columns,
            tableData,
            state.filterList,
            '',
        ...
        const mountWrapper = mount(
            shallow(
                <MUIDataTable
                    ...
                    data={data}
                    options={{
                        rowsPerPageOptions: [2, 4, 10, 15, 100],
                        rowsPerPage: 2
                    }}
                />
            ).get(0)
    ...
    it('should correctly call consoleWarnings', () => {
        ...
        const shallowWrapper3 = shallow(
            <MUIDataTable columns={columns} data={data} options={options3} />
        ).dive()
    ...
    it('should call onRowExpansionChange when row is expanded or collapsed', () => {
        ...
        assert(
            options.onRowExpansionChange.calledWith(
                [{ index: 2, dataIndex: 2 }],
                [{ index: 2, dataIndex: 2 }]
            )
        )
    ...
    it('should call onRowSelectionChange when row is selected or unselected', () => {
        ...
        assert(
            options.onRowSelectionChange.calledWith(
                [{ index: 2, dataIndex: 2 }],
                [{ index: 2, dataIndex: 2 }]
            )
        )
    ...
    describe('should displayData consider filterOptions with logic', () => {
        it('with one column', () => {
            ...
            const shallowWrapper = shallow(
                <MUIDataTable columns={customFilterColumns} data={data} />
            )
            const table = shallowWrapper.dive()
            const instance = table.instance()
    ...
    describe('should correctly handle non-array data', () => {
        ...
        it('should correctly filter data when no array data is present', () => {
            ...
            const expectedResult = JSON.stringify([
                { data: ['other-data-2', 'b'], dataIndex: 1 }
            ])
            assert.deepEqual(JSON.stringify(displayData), expectedResult)
        })
    ...
    describe('should correctly run comparator function', () => {
        it('correctly compares two equal strings', () => {
            expect(
                getCollatorComparator()('testString', 'testString')
            ).to.equal(0)
        })

        it('correctly compares two different strings', () => {
            expect(
                getCollatorComparator()('testStringA', 'testStringB')
            ).to.equal(-1)
        })
    })
